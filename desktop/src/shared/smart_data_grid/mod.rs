...
impl SmartDataGrid {
    pub fn new(
        ...
        let blur_sub = cx.on_blur(&focus_handle, window, |this: &mut Self, window, cx| {
            match this.commit_edit(window, cx) {
                Ok((row_ix, col_ix)) => {
    ...
    fn commit_edit(
        ...
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> Result<(usize, usize), CommittingError> {
        self.table.update(cx, |table, cx| {
            let delegate = table.delegate_mut();
            let (r, c) = match &delegate.state.editing_state {
                Some(s) => (s.row, s.col),
                None => return Err(CommittingError::NoActiveEdit),
            };
            ...
            if validate_sql_type(&value, &col_type) {
                ...
                cx.notify();
                Err(CommittingError::InvalidData(format!(
    ...
    fn on_confirm_edit(&mut self, _: &ConfirmEdit, window: &mut Window, cx: &mut Context<Self>) {
        match self.commit_edit(window, cx) {
            Ok((row_ix, col_ix)) => {
                self.table.update(cx, |table, cx| {
