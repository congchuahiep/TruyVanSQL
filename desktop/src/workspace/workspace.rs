...
impl Workspace {
    pub fn new(_window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let title_bar = cx.new(|cx| Titlebar::main(cx));
    ...
    fn on_new_database(&mut self, _action: &NewDatabase, _w: &mut Window, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |_, cx| {
            ...
            if let Some(file) = dialog.save_file().await {
                ...
                let name = SharedString::from(file.file_name());
            ...
            cx.open_window(
                WindowOptions {
                    ...
                    titlebar: Some(TitlebarOptions {
                        title: Some("Kết nối PostgreSQL".into()),
                ...
                |window, cx| {
                    ...
                    let view = cx.new(|cx| ConnectionWindow::new(connection_store, window, cx));
                    cx.new(|cx| Root::new(view, window, cx))
                },
            )
            .expect("Không thể mở cửa sổ kết nối");
        })
        .detach();
    }
    ...
    fn register_actions(&self, el: Div, cx: &mut Context<Self>) -> Div {
        el.key_context("app")
            ...
            .on_action(cx.listener(Self::on_connect_database))
