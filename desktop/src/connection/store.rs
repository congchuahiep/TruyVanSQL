...
/// Dịch vụ quản lý Workspace (Connection Manager cũ).
...
impl ConnectionStore {
    ...
    pub fn add_connection(&mut self, name: String, config: engine::DatabaseConfig, cx: &mut Context<Self>) {
    ...
    pub fn active_connection(&self) -> Option<Entity<DatabaseConnection>> {
        self.active_index.and_then(|i| self.connections.get(i).cloned())
    }
